...
#include "argparser/argparser_argument_mgr.h"
		...
		void *all_chars_)
{
	...
	if (argparser_argument->char_key)
		all_chars[ft_strlen(all_chars)] = argparser_argument->char_key;
}
...
char				*argparser_argument_mgr_get_all_chars(t_lst *arguments)
{
	...
	all_chars = ft_strnew(lst_len(arguments));
	lst_iter(arguments, print_argparser_argument_fn, all_chars);
	return (all_chars);
}
